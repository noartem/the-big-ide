...
description: Repo-local workflow skill for large coding tasks. Initializes .agent/tasks/TASK_ID artifacts, installs project-scoped OpenCode subagents, updates AGENTS.md with the workflow, and runs a spec-freeze -> build -> evidence -> verify -> fix loop with fresh-session verification.
license: Apache-2.0
compatibility: Skills-compatible coding agents. Integrates with OpenCode project-scoped subagents. Bundled scripts require Python 3.10+.
metadata:
  author: OpenAI
  version: "1.0.0"
---
...
3. Installs project-scoped OpenCode subagent templates into `.opencode/agents/`
4. Updates `AGENTS.md` with a managed block that explains the workflow
...
- `init <TASK_ID>`: create `.agent/tasks/<TASK_ID>/`, install or refresh subagent templates, and update `AGENTS.md`
...
- In OpenCode, prefer the installed project subagents from `.opencode/agents/`.
...
- Preserve existing user guidance outside the managed block in `AGENTS.md`
