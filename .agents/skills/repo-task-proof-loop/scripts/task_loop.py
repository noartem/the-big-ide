...
def update_guides(repo_root: Path, guides: str) -> dict[str, str]:
    ...
    if guides in {"auto", "agents"}:
        guide_targets.append((repo_root / "AGENTS.md", load_text_template("managed-block-agents.md.tmpl")))

    if guides == "auto":
        guide_targets = [(repo_root / "AGENTS.md", load_text_template("managed-block-agents.md.tmpl"))]
...
def cmd_init(args: argparse.Namespace) -> int:
    ...
    if args.install_subagents == "opencode":
